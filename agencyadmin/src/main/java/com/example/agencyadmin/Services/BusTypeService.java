 ...
 * All business logic related to bus types should be implemented in this
 * service.
 */
@Service
public class BusTypeService {
    ...
    public List<BusTypeDTO> getBusTypesByAgency(String agencyId) {
        List<BusType> busTypes = busTypeRepository.findByAgencyid(agencyId);
        return busTypes.stream().map(busTypeMapper::toDTO).toList();
    }
    ...
    public Optional<BusTypeDTO> updateBusType(UUID busTypeId, BusTypeDTO busTypeDTO) {
        ...
        if (existingBusType.isPresent()) {
            ...
            busType.setAgencyid(busTypeDTO.getAgencyId());
